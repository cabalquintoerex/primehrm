...
-- This touches ONLY the new `eprime` database — the other apps' databases and
-- users are not referenced, so it is safe on the shared instance.
-- ---------------------------------------------------------------------------

CREATE DATABASE IF NOT EXISTS `eprime`
  CHARACTER SET utf8mb4
  COLLATE utf8mb4_unicode_ci;

-- Change the password, and keep it in sync with DATABASE_URL in server/.env.
CREATE USER IF NOT EXISTS 'eprime'@'localhost'
  IDENTIFIED BY 'CHANGE_ME_STRONG_PASSWORD';

-- Rights scoped to this database only.
GRANT ALL PRIVILEGES ON `eprime`.* TO 'eprime'@'localhost';
